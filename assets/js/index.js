const esquemadepropiedades = document.querySelector(".propiedades");
...
const Inicio = () => {
  ...
  esquemadepropiedades.innerHTML += html;
  total.innerHTML = propiedades.length;
}
const busquedaItem = (desde, hasta, cuartos) => {
  html = "";
  let resultadoBusqueda = [];
  for (const item of propiedades) {
    if ((desde <= item.metros <= hasta) && (item.cuartos >= cuartos)) {
      html += esquema (item);
  ...
  if(resultadoBusqueda.length>0) {
    ...
    esquemadepropiedades.innerHTML = `
...
btn.addEventListener("click", validar = () => {
  ...
  var desde = document.querySelector("#inicio").value;
  var hasta = document.querySelector("#termino").value;

  if (cuartos != 0 && dsd != 0 && hst != 0) {
    busquedaItem(desde, hasta, cuartos);
  }
});
